    ...
    public partial class SearchControl : System.Web.UI.UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if(Request.QueryString["HomeType"]!=null&&Request.QueryString["HomeType"]!="")
                {
                    Session["HomeType"] = Request.QueryString["HomeType"];
                }
        ...
        void SearchResidentialProperties()
        {
            try
            {
                    ...
                    {
                    if(Session["HomeType"]!=null&&Session["HomeType"].ToString()!="")
                    {
                        dt = mlsClient.GetResidentialProperties("0",Session["HomeType"].ToString(), "0", "0", "0", "0", "0");
                    }
                    else
                    {
        ...
        void SearchCommercialProperties()
        {
            try
            {
                ...
                if (Convert.ToString(Session["QueryString"]) == "Commercial" && Session["Municipality"] == null)
                {
                    if (Session["HomeType"] != null && Session["HomeType"].ToString() != "")
                    {
                        dt = mlsClient.GetAllCommercialProperties("0", Session["HomeType"].ToString(), "0", "0", "0",  "0");
                    }
                    else
                    {
            ...
            try
            {
                ...
                if (Convert.ToString(Session["QueryString"]) == "Condo" && Session["Municipality"] == null)
                {
                    if (Session["HomeType"] != null && Session["HomeType"].ToString() != "")
                    {
                        ...
                        dt = mlsClient.GetProperties_Condo("0", Session["HomeType"].ToString(),"0", "0", "0", "0", "0");
                    }
                    else
                    {
